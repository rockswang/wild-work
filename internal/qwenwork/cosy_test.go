...
// 使签名内的时间戳与头里的不一致 → 上游回 101 Signature invalid
// （长会话里偶发，body 越大越频繁）。
//
...
func TestCosyDateMatchesSignature(t *testing.T) {
	...
	s := &CosySession{CosyKey: "cosykey", Info: "info"}
	const body = `{"hello":"world"}`
	const rawURL = "https://gateway.qwenwork.cn/algo/api/v2/service/pro/sse/agent_chat_generation?FetchKeys=llm_model_result"
	const wantPath = "/api/v2/service/pro/sse/agent_chat_generation"

	h := map[string]string{}
	if err := s.ApplyHeadersWithUID(h, body, rawURL, "uid"); err != nil {
		t.Fatal(err)
	}
